...
export const useConversationAutoplay = (
  ...
  const stopAutoplay = useCallback(() => {
    AppLogger.debug("🛑 User interrupted autoplay");
  ...
  const playNextUnreadMessage = useCallback(() => {
    ...
    if (userInterruptedRef.current) {
      AppLogger.debug("⏸️ Autoplay stopped - user interrupted");
      setIsAutoPlaying(false);
      return;
    }
    ...
    const sortedMessages = sortBy(conversation.messages, (m) =>
      m.timestamp.getTime()
    );

    // Find the current message by matching audioUrl
    AppLogger.debug(
      "🔍 playNextUnreadMessage - currentUriRef:",
    ...
    if (currentUriRef.current === "" || isInitializingRef.current) {
      AppLogger.debug(
        "🔍 currentUriRef is empty or initializing, forcing find oldest unread"
      );
      const unreadFromOther = sortedMessages.filter(
        (m) => m.uid === otherProfile.uid && !m.isRead
      );
      AppLogger.debug(
        "🔍 Finding oldest unread - currentUriRef:",
        currentUriRef.current,
        "unread count:",
        unreadFromOther.length
      );
      if (unreadFromOther.length > 0) {
        const oldestUnread = unreadFromOther[0];
        AppLogger.debug(
          "▶️ Playing oldest unread message:",
          oldestUnread.messageId,
          "out of",
          unreadFromOther.length,
          "unread messages"
        );
      ...
      AppLogger.debug("❌ No unread messages found");
      setIsAutoPlaying(false);
      return;
    }
    ...
    AppLogger.debug(
      "🔍 playNextUnreadMessage - currentMessageIndex:",
      currentMessageIndex
    );
    ...
    if (currentMessageIndex === -1) {
      ...
      AppLogger.debug("❌ Current message not found and no unread messages");
      setIsAutoPlaying(false);
      return;
    }
    ...
    if (currentMessage && currentMessage.uid === profile.uid) {
      AppLogger.debug("⚠️ Current message is from sender, stopping autoplay");
      setIsAutoPlaying(false);
      return;
    }
    ...
    for (let i = currentMessageIndex + 1; i < sortedMessages.length; i++) {
      ...
      if (nextMessage.isRead) {
        AppLogger.debug("✅ Next message is read, stopping auto-play");
        setIsAutoPlaying(false);
        return;
      }
      ...
      if (userInterruptedRef.current) {
        AppLogger.debug(
          "⏸️ Autoplay stopped before playing next message - user interrupted"
        );
        setIsAutoPlaying(false);
        return;
      }
      AppLogger.debug(
        "▶️ Playing next unread message:",
      ...
      setTimeout(() => {
        ...
        if (!userInterruptedRef.current) {
          ...
          AppLogger.debug(
            "⏸️ Autoplay stopped during delay - user interrupted"
          );
          setIsAutoPlaying(false);
        }
    ...
    AppLogger.debug("✅ No more unread messages");
    setIsAutoPlaying(false);
  ...
  useEffect(() => {
    ...
    if (currentMessageCount > lastMessageCountRef.current) {
      ...
      if (newestMessage && newestMessage.uid === otherProfile.uid) {
        ...
        if (unheardMessages.length > 0) {
          ...
          if (newestUnheard.uid !== profile.uid) {
            ...
            if (lastPlayedMessageIdRef.current !== newestUnheard.messageId) {
              AppLogger.debug(
                "🔔 New message received, autoplaying:",
            ...
            AppLogger.debug(
              "⚠️ Attempted to autoplay sender's own message, blocking"
            );
          }
        }
      } else {
        ...
        AppLogger.debug(
          "📤 New message sent by current user, skipping autoplay"
        );
        if (currentUriRef.current) {
          ...
          if (currentMessage && currentMessage.uid === profile.uid) {
            AppLogger.debug(
              "🛑 Clearing currentUri to prevent playing sender's own message"
            );
            setCurrentUri("");
            currentUriRef.current = "";
            setIsAutoPlaying(false);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (!userInterruptedRef.current) {
        ...
        AppLogger.debug("🎬 Starting autoplay from oldest unread message");
  ...
  useEffect(() => {
    if (!autoplay) {
      AppLogger.debug("🔴 Autoplay disabled - stopping autoplay");
      setIsAutoPlaying(false);
      ...
      AppLogger.debug("🟢 Autoplay enabled - resetting state");
